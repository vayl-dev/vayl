...
<p align="center">
  <img src="docs/vayl-demo.gif" alt="Live Vayl demo on a local model: plain sentences are stored via the vayl-mcp server, then recall returns the current answer — 'what do we use for state now?' returns Zustand; 'are we still using Redux?' returns no, we switched to Zustand." width="860">
</p>
...
**TypeScript** — the same client for TS/JS agents (`npm i @vayl.dev/client`, source in [`clients/typescript/`](clients/typescript/)): `const m = await Vayl.connect({ userId: "proj_7" }); await m.remember(...); await m.recall(...)`.
